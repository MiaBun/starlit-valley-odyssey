console.info("[STARLIT] checkAttributes.js loaded");
    ...
    (e) => {
        ...
        if (playerAttributes) {
            ...
            if (binPlayer) {
                player.tell(
                    Text.translatable(
                        "starlit.shipping_bin.owner",
                        Text.of(`${binPlayer.username}`).gold()
                    )
                );

                player.tell(Text.translatable("starlit.shipping_bin.sell_multipliers").gray());

                player.tell(
                    Text.translatable(
                        "starlit.shipping_bin.multiplier", 
                        Text.translatable("tooltip.starlit.farmer_product"), 
                ...
                player.tell(
                    Text.translatable(
                        "starlit.shipping_bin.multiplier", 
                        Text.translatable("tooltip.starlit.artisan_product"), 
                ...
                player.tell(
                    Text.translatable(
                        "starlit.shipping_bin.multiplier", 
                        Text.translatable("tooltip.starlit.geologist_product"), 
                        Text.green(`${gem[0] ? gem[0].Base : 1}`)
                    )
                );
                ...
                player.tell(
                    Text.translatable(
                        "starlit.shipping_bin.multiplier", 
                        Text.translatable("tooltip.starlit.adventurer_product"), 
            ...
            player.tell(
                Text.translatable("starlit.shipping_bin.offline").gray()
            );
        }
    }
);