...
StartupEvents.registry("block", (event) => { //TODO: wiki
  event
    ...
    .item((item) => {
      ...
      item.tooltip(
        Text.translatable("tooltip.starlit.smart_shipping_bin.warn").red()
      );
      item.modelJson({
        parent: "starlit:block/kubejs/smart_shipping_bin",
      });
    })
    .blockEntity((blockInfo) => {
      ...
      blockInfo.serverTick(4000, 0, (entity) => {
        ...
        if (global.susFunctionLogging) console.log("[STARLIT-SUSFN] smartShippingBin.js");
    ...
    multipart: [
      {
        when: { facing: "north" },
        apply: {
          model: "starlit:block/kubejs/smart_shipping_bin",
          y: 0,
          uvlock: false,
        },
      },
