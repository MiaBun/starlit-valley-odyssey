  ...
  for (let i = 0; i < inventorySlots; i++) {
    ...
    if (isSellable) {
      ...
      if (
        ...
        (Item.of(slotItem).hasTag("starlit:artifacts") ||
          Item.of(slotItem).hasTag("starlit:relics"))
      ) {
        itemValue *= 3;
      }
      if (
        stages.toString().includes("brine_and_punishment") &&
        Item.of(slotItem).hasTag("starlit:brine_and_punishment")
      ) {
        itemValue *= 2;
      }
...
global.handleShippingBinDebt = (
  ...
  if (debtPaid > 0) {
    let receiptAuthor = global.translatableWithFallback("starlit.hospital_receipt.author", "Starlit Valley Hospital").getString();
    let receiptText = Text.translatable(
      "starlit.shipping_bin.debt_paid_note",
      player.username,
      global.formatPrice(debtPaid.toFixed()),
